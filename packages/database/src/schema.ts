...
export const appointmentSource = pgEnum("appointment_source", ["public_web", "dashboard", "import"]);
export const actorType = pgEnum("actor_type", ["customer", "member", "system", "platform_admin"]);
export const exceptionKind = pgEnum("exception_kind", ["closed", "available"]);
export const outboxStatus = pgEnum("outbox_status", ["pending", "processing", "delivered", "dead"]);
...
export const users = pgTable(
  ...
  (table) => [
    uniqueIndex("users_oidc_identity_uq").on(table.oidcIssuer, table.oidcSubject),
...
export const tenants = pgTable(
  ...
  (table) => [
    ...
    check("tenants_slug_format_ck", sql`${table.slug} ~ '^[a-z0-9](?:[a-z0-9-]{1,48}[a-z0-9])$'`),
...
export const bookingSettings = pgTable("booking_settings", {
  ...
  check("booking_settings_interval_ck", sql`${table.slotIntervalMinutes} between 5 and 240`),
  check("booking_settings_lead_ck", sql`${table.minimumLeadMinutes} between 0 and 525600`),
  check("booking_settings_horizon_ck", sql`${table.bookingHorizonDays} between 1 and 730`),
]);
...
export const locations = pgTable(
  ...
  (table) => [index("locations_tenant_active_idx").on(table.tenantId, table.isActive)],
);
...
export const staffMembers = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("staff_members_tenant_user_uq").on(table.tenantId, table.userId),
  ],
);
...
export const services = pgTable(
  ...
  (table) => [
    ...
    check("services_duration_ck", sql`${table.durationMinutes} between 5 and 1440`),
    check("services_buffers_ck", sql`${table.bufferBeforeMinutes} >= 0 and ${table.bufferAfterMinutes} >= 0`),
...
export const serviceLocations = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.serviceId, table.locationId] }), index("service_locations_tenant_idx").on(table.tenantId)],
);
...
export const staffServices = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.staffId, table.serviceId] }), index("staff_services_tenant_idx").on(table.tenantId)],
);
...
export const weeklyAvailability = pgTable(
  ...
  (table) => [
    index("weekly_availability_lookup_idx").on(table.tenantId, table.locationId, table.staffId, table.dayOfWeek),
    check("weekly_availability_day_ck", sql`${table.dayOfWeek} between 0 and 6`),
    check("weekly_availability_time_ck", sql`${table.startsLocalAt} < ${table.endsLocalAt}`),
  ],
);
...
export const availabilityExceptions = pgTable(
  ...
  (table) => [
    index("availability_exceptions_lookup_idx").on(table.tenantId, table.locationId, table.staffId, table.startsAt),
    check("availability_exceptions_range_ck", sql`${table.startsAt} < ${table.endsAt}`),
  ],
);
...
export const customers = pgTable(
  ...
  (table) => [
    uniqueIndex("customers_tenant_email_uq").on(table.tenantId, sql`lower(${table.email})`).where(sql`${table.email} is not null`),
...
export const appointments = pgTable(
  ...
  (table) => [
    ...
    index("appointments_tenant_status_start_idx").on(table.tenantId, table.status, table.startsAt),
    index("appointments_tenant_staff_start_idx").on(table.tenantId, table.staffId, table.startsAt),
    index("appointments_tenant_location_start_idx").on(table.tenantId, table.locationId, table.startsAt),
...
export const appointmentServices = pgTable(
  ...
  (table) => [index("appointment_services_appointment_idx").on(table.tenantId, table.appointmentId)],
);
...
export const appointmentEvents = pgTable(
  ...
  (table) => [index("appointment_events_history_idx").on(table.tenantId, table.appointmentId, table.occurredAt)],
);
...
export const idempotencyKeys = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.tenantId, table.key] }), index("idempotency_keys_expiry_idx").on(table.expiresAt)],
);
