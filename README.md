...
Set `RATE_LIMIT_SECRET` to a separate random value of at least 32 characters. Only an HMAC of the normalized business slug and proxy-supplied client address is persisted in rate-limit counters. Vercel deployments use the platform-overwritten `x-vercel-forwarded-for` header. For self-hosting, put a reverse proxy in front of Next.js, configure it to strip any inbound value and overwrite a single client-address header, then set `TRUSTED_PROXY_CLIENT_IP_HEADER` to that header's name. Without this setting, self-hosted traffic intentionally shares a conservative 20-request-per-minute bucket per tenant; the application never trusts `x-forwarded-for` by default.
